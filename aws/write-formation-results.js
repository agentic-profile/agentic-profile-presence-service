 ...
 *
 * Usage:
 *   node aws/write-formation-results.js --stage=staging
 *   node aws/write-formation-results.js --stage=prod
 *
 */
...
function main() {
  ...
  if (!stage) {
    console.error('Missing stage. Pass it on the command line: --stage=staging or --stage=prod');
    process.exit(1);
  }
  ...
  if (!stackBaseName) {
    console.error('Missing StackName. Set it in "cloud.env"');
    process.exit(1);
  }
