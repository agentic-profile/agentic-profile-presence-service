...
export function createPresenceRouter( didResolver: Resolver, store: UnifiedStore ) {
    ...
    async function handleMcpRequest(req: Request, res: Response) {
        await processJsonRpcMethod( req, res, didResolver, store,
            async ( jrpcRequest: JsonRpcRequest, session: ClientAgentSession | null): Promise<JsonRpcResponse | null> => {
                ...
                switch( jrpcRequest.method ) {
                    ...
                    case 'tools/call':
                        /*if( !session )
                            ...
                            return await handleToolsCall( jrpcRequest as JSONRPCRequest, session!, req, store );
                    default:
                        return null;
                }
            }
